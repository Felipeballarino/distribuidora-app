          ...
          {items.map((item) => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-4 mt-1 text-sm text-texto-suave">
                  <span>{formatearCantidad(item.cantidad, item.unidad_medida)}</span>
                  <span>×</span>
                  <span className="font-mono">{formatearPrecio(item.precio_con_iva)}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => actualizarCantidad(item.cod_articulo, item.cantidad - (item.unidad_medida === 'kg' ? 0.5 : 1))}
                  className="w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-lg font-bold"
                >
                  −
                </button>
                <span className="w-12 text-center font-bold font-mono">
              ...
              <p className="text-xl font-extrabold font-mono text-texto w-28 text-right">
                {formatearPrecio(item.precio_con_iva * item.cantidad)}
              </p>
